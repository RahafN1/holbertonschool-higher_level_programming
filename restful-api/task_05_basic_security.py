#!/usr/bin/python3
"""
A simple Flask API demonstrating Basic Authentication and
JWT-based authentication, including role-based access control.

Supported endpoints:
    GET  /basic-protected  -> Protected with HTTP Basic Auth
    POST /login             -> Exchanges valid credentials for a JWT token
    GET  /jwt-protected     -> Protected with a valid JWT token
    GET  /admin-only        -> Protected with a valid JWT token + admin role
"""
...
@auth.login_required
def basic_protected():
    """Return a success message once Basic Auth credentials are valid."""
...
@jwt_required()
def jwt_protected():
    """Return a success message once a valid JWT token is provided."""
